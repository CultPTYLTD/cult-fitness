...
import { Camera, Barcode, Loader2, X, Check } from 'lucide-react';
...
export const FoodScanner = ({ open, onClose, onScanComplete }: FoodScannerProps) => {
  const [mode, setMode] = useState<'select' | 'camera' | 'barcode'>('select');
      ...
      <DialogContent className="max-w-md mx-auto">
        <DialogHeader>
          <DialogTitle className="text-center font-serif">
            {result ? 'Nutrition Info' : mode === 'select' ? 'Scan Food' : mode === 'camera' ? 'Take Photo' : 'Enter Barcode'}
          </DialogTitle>
        </DialogHeader>
