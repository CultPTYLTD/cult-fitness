...
export function DailyReflectionModal({ open, onClose }: DailyReflectionModalProps) {
  ...
  const reflectionCards = [
    { id: "hydration", label: "HYDRATION", value: `${hydration} of ${hydrationGoal} ml`, progress: (hydration / hydrationGoal) * 100, bgClass: "bg-gradient-to-r from-blue-200/80 to-blue-100/60" },
    { id: "steps", label: "STEPS", value: `${steps.toLocaleString()} of ${stepsGoal.toLocaleString()}`, progress: (steps / stepsGoal) * 100, bgClass: "bg-gradient-to-r from-green-200/80 to-green-100/60" },
    { id: "sleep", label: "SLEEP", value: `${sleepHours} of ${sleepGoal} hours`, progress: (sleepHours / sleepGoal) * 100, bgClass: "bg-gradient-to-r from-purple-200/80 to-purple-100/60" },
  ];
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="p-0 gap-0 max-w-md border-0 overflow-hidden bg-background max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-6 pb-6">
          <h2 className="text-3xl font-serif font-light text-foreground mb-4">DAILY REFLECTION</h2>
            ...
            {dayLabels.map((day, i) => (
              <button
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${
                  selectedDay === i ? "bg-black text-white" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                {day}
              </button>
          ...
          <div className="space-y-3 mb-6">
            {reflectionCards.map((card) => (
              <div key={card.id} className={`${card.bgClass} rounded-2xl p-4`}>
          ...
          <div className="flex justify-between mb-8">
            {macros.map((macro) => (
              <div key={macro.label} className="flex flex-col items-center">
                <div className="w-14 h-14 rounded-full border-2 border-black/30 flex items-center justify-center mb-2">
          ...
          <Button className="w-full" size="lg" onClick={handleComplete}>COMPLETE MY DAY</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
